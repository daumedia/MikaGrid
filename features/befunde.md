...
|---|---|---|---|---|---|
| BF-08 | 01 | **`NSAppleEventsUsageDescription` ist deutsch, die App englisch.** `Resources/Info-MAS.plist` trägt „Mika+Grid lässt Kurzbefehle Fenster verschieben…" — dieser Satz erscheint im Systemdialog, mit dem macOS die Erlaubnis erfragt, auch der App-Prüfung. Oberfläche, Website und Store-Eintrag sind englisch | mittel | **Offen.** Unklare Zweckangaben sind ein bekannter Ablehnungsgrund (Guideline 5.1.1). Vorschlag: Schlüssel auf Englisch, `CFBundleDevelopmentRegion` auf `en`, eine deutsche Fassung später über `de.lproj/InfoPlist.strings`. Steht als Entscheidung in `AppStore/CHECKLISTE.md` | 2026-08-26 |
| BF-09 | 01 | **Das Popover zeigt elf Zonen, die Store-Fassung führt zehn aus.** `PopoverGridView.snapGrid` rendert `.restore` unbedingt; `ShortcutsWindowSnapper:63` beantwortet die Aktion mit `.nothingToRestore` | niedrig | **Offen, entschärft.** Die Zone ist mit „Nothing to restore" ehrlich beschriftet, es entsteht kein stiller Fehlschlag. Bleibt ein Widerspruch zwischen Store-Screenshot (elf Zonen) und Beschreibung (zehn Aktionen). Entweder die Zone in der Store-Fassung ausgrauen oder so belassen; die ausgelieferten Screenshots heben sie nicht hervor. Steht als Entscheidung in `AppStore/CHECKLISTE.md` | 2026-08-26 |
